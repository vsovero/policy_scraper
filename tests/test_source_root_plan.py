...
def test_build_source_root_plan_uses_unc_official_archive_first():
    plan = build_source_root_plan(institutions())

    unc_rows = plan[plan["unitid"].eq(199139)]

    preferred = unc_rows.sort_values("fallback_order").iloc[0]
    assert preferred["source_root_name"] == "UNC Charlotte Provost Catalog Nodes"
    assert preferred["first_pass_decision"] == "use_for_first_pass_with_archive_bounds"
    digitalnc = unc_rows[unc_rows["source_root_name"].str.contains("DigitalNC")].iloc[0]
    assert digitalnc["source_root_role"] == "deferred_external_archive"
