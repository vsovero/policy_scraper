...
- preserve official institution-wide academic policy pages as deferred policy-extraction leads when they contain the course repetition rule, but keep Phase 3 catalog-first because current policy pages create historical dating problems for AY 2000-2020;
...
- `catalog_pilot_status_summary_strict_pilot.csv` provides a one-row-per-institution dashboard with coverage, unresolved years, escalation bucket, and recommended next action;
...
- `catalog_batch2_*` files set up the next 5-institution expansion batch using the finalized source-root protocol. They identify selected institutions, legacy leads, candidate source roots, preferred root decisions, institution-year setup statuses, and the batch dashboard; they do not count catalog coverage yet.
