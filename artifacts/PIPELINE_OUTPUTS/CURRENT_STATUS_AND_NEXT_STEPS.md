...
Packet 013-016 is the latest Step 1 URL-stage prior-discovery source reconstruction packet accepted by process review. It adds 112 institutions, 1,603 target institution-years, 697 accepted source-ledger rows, 906 explicit unresolved rows, 518 benchmark rows, 453 current-run benchmark recoveries, 65 benchmark rows invalidated by review, 0 benchmark rows resolved by other source-ledger evidence, and 0 unresolved benchmark misses.

The dtype-handling fix from the packet review has been integrated on `main` as `03aa2b6`.

## Current Production-Construction Totals

- Accepted batches: 16 (001-016)
...
Run the next managed testing packet, `step1_prior_discovery_source_reconstruction_packet_017_020`, from clean `origin/main` after this packet-status update. The testing packet should run batches 017-020 sequentially, preserve run-local artifacts, avoid project-management docs, and hand off to a packet-level process review.
...
- Review records for batches 001-016 were produced in their batch worktrees; publishing ignored review artifacts into canonical `process_reviews/` remains a review-stream task, not a project-management task.
